...
use CreditAgricole\Etransactions\Helper\Utf8Data;
...
class Creditagricole
{
    ...
    protected function _callDirect($type, $amount, Order $order, Transaction $transaction)
    {
        ...
        $client = new \GuzzleHttp\Client([
            ...
            'headers' => [
                'User-Agent' => 'Magento Up2pay e-Transactions module',
            ],
            'timeout' => 5,
        ]);
    ...
    public function buildSystemParams(Order $order, AbstractPayment $payment)
    {
        ...
        if ($payment->getCode() == 'etep_threetime') {
            ...
            switch ($payment->getCreditagricoleAction()) {
                ...
                case AbstractPayment::ETRANSACTION_DEFERRED:
    ...
    protected function checkSignature($data, $keyFile = 'pubkey.pem')
    {
        ...
        if (!preg_match('#^(.*)&K=(.*)$#', $data, $matches)) {
            throw new \LogicException(sprintf('Error Processing Request: %s', __('An unexpected error in Up2pay e-Transactions call has occured: missing signature.')));
        }
    ...
    public function getParams($logParams = false, $checkSign = true)
    {
        ...
        if (empty($data)) {
            throw new \LogicException(sprintf('Error Processing Request: %s', __('An unexpected error in Up2pay e-Transactions call has occured: no parameters.')));
        }
        ...
        if ($checkSign) {
            ...
            if (!$this->checkSignature($data) && !$this->checkSignature($data, 'pubkey_1024.pem')) {
                throw new \LogicException(sprintf('Error Processing Request: %s', __('An unexpected error in Up2pay e-Transactions call has occured: invalid signature.')));
            }
        }
